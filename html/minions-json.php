...
$result = doQuery($mysqli, "SELECT `server_id`, `id`, `host`, `fqdn`, `biosversion`, `biosreleasedate`, `cpu_model`, `kernel`, `kernelrelease`, `os`, `osrelease`, `saltversion`, `last_seen`, `last_audit`, `last_seen`, (SELECT COUNT(`package_id`) FROM `minion_package` WHERE `minion_package`.`server_id` = `minion`.`server_id`) AS `packages` FROM `minion`;");
