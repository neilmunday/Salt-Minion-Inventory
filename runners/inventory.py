...
def __connect():
    ...
    CONFIG_FILE = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "inventory.ini"
    )
    ...
    log.debug(
        "inventory.__connect: connection to %s on %s as %s" % (dbName, dbHost, dbUser)
    )
    try:
        db = MySQLdb.connect(
            user=dbUser,
...
def __getTimeStamp(tsStr):
    ...
    return int(
        pytz.timezone("UTC")
        .localize(datetime.datetime.strptime(tsStr, "%Y-%m-%dT%H:%M:%S.%f"))
        .timestamp()
    )
...
def __getVendorId(db, cursor, vendor):
    ...
    """
    __doQuery(
        cursor, "SELECT `vendor_id` FROM `vendor` WHERE `vendor_name` = '%s';" % vendor
    )
    if cursor.rowcount == 0:
        # add new vendor
        __doQuery(
            cursor, "INSERT INTO `vendor` (`vendor_name`) VALUES ('%s');" % vendor
        )
...
def audit(ts, properties, propertiesChanged):
    ...
    # get server id
    serverId = __getRecordId(
        cursor, "minion", "server_id", "server_id", properties["server_id"]
    )
    if not propertiesChanged:
        ...
        __doQuery(
            cursor,
            'UPDATE `minion` SET `last_audit` = "%s" WHERE `server_id` = %d;'
            % (ts, serverId),
        )
        db.commit()
        return True
    ...
    __doQuery(
        cursor,
        'SELECT `server_model_id` FROM `server_model` WHERE `vendor_id` = %d AND `server_model` = "%s" LIMIT 0,1;'
        % (vendorId, properties["productname"]),
    )
    modelId = 0
    if cursor.rowcount == 0:
        __doQuery(
            cursor,
            'INSERT INTO `server_model` (`server_model`, `vendor_id`) VALUES ("%s", %d);'
            % (properties["productname"], vendorId),
        )
        db.commit()
    ...
    __doQuery(
        cursor,
        "UPDATE `minion_user` SET `present` = 0 WHERE `server_id` = %d;" % serverId,
    )
    db.commit()
    for user in properties["users"]:
        ...
        __doQuery(
            cursor,
            "INSERT INTO `minion_user` (`server_id`, `user_id`, `present`) VALUES (%d, %d, 1) ON DUPLICATE KEY UPDATE `present` = 1;"
            % (serverId, userId),
        )
        db.commit()
    ...
    __doQuery(
        cursor,
        "UPDATE `minion_disk` SET `present` = 0 WHERE `server_id` = %d;" % serverId,
    )
    db.commit()
    for disk in properties["disks"]:
        ...
        __doQuery(
            cursor,
            "INSERT INTO `minion_disk` (`server_id`, `disk_path`, `disk_serial`, `disk_size`, `vendor_id`, `present`) VALUES (%d, '%s', '%s', %d, %d, 1) ON DUPLICATE KEY UPDATE `present` = 1, `disk_serial` = '%s', `disk_size` = %d, `vendor_id` = %d;"
            % (
                serverId,
                disk["name"],
                disk["serial"],
    ...
    __doQuery(
        cursor,
        "UPDATE `minion_gpu` SET `gpu_qty` = 0 WHERE `server_id` = %d;" % serverId,
    )
    db.commit()
    for gpu in properties["gpus"]:
        ...
        __doQuery(
            cursor,
            'SELECT `gpu_id` FROM `gpu` WHERE `gpu_model` = "%s" AND `vendor_id` = %d;'
            % (gpu["model"], vendorId),
        )
        if cursor.rowcount == 0:
            # add new GPU
            __doQuery(
                cursor,
                'INSERT INTO `gpu` (`gpu_model`, `vendor_id`) VALUES ("%s", %d);'
                % (gpu["model"], vendorId),
            )
        ...
        __doQuery(
            cursor,
            "INSERT INTO `minion_gpu` (`server_id`, `gpu_id`, `gpu_qty`) VALUES (%d, %d, 1) ON DUPLICATE KEY UPDATE `gpu_qty` = `gpu_qty` + 1;"
            % (serverId, gpuId),
        )
        db.commit()
    # delete removed GPUs
    ...
    __doQuery(
        cursor,
        "UPDATE `minion_interface` SET `present` = 0 WHERE `server_id` = %d;"
        % serverId,
    )
    db.commit()
    __doQuery(
        cursor,
        "UPDATE `minion_ip4` SET `present` = 0 WHERE `server_id` = %d;" % serverId,
    )
    db.commit()
    for interface, addr in __items(properties["hwaddr_interfaces"]):
        if interface != "lo":
            interfaceId = __getRecordId(
                cursor, "interface", "interface_id", "interface_name", interface
            )
            if not interfaceId:
                __doQuery(
                    cursor,
                    'INSERT INTO `interface` (`interface_name`) VALUES ("%s");'
                    % interface,
                )
            ...
            __doQuery(
                cursor,
                'INSERT INTO `minion_interface` (`server_id`, `interface_id`, `mac`, `present`) VALUES (%d, %d, "%s", 1) ON DUPLICATE KEY UPDATE `present` = 1, `mac` = "%s";'
                % (serverId, interfaceId, addr, addr),
            )
            db.commit()
            if interface in properties["ip4_interfaces"]:
                if len(properties["ip4_interfaces"][interface]) > 0:
                    for ip in properties["ip4_interfaces"][interface]:
                        __doQuery(
                            cursor,
                            'INSERT INTO `minion_ip4` (`server_id`, `interface_id`, `ip4`, `present`) VALUES (%d, %d, "%s", 1) ON DUPLICATE KEY UPDATE `present` = 1;'
                            % (serverId, interfaceId, ip),
                        )
                        db.commit()

    __doQuery(
        cursor,
        "DELETE FROM `minion_ip4` WHERE `server_id` = %d AND `present` = 0;" % serverId,
    )
    db.commit()
    __doQuery(
        cursor,
        "DELETE FROM `minion_interface` WHERE `server_id` = %d AND `present` = 0;"
        % serverId,
    )
    db.commit()
    ...
    __doQuery(
        cursor,
        "DELETE FROM `minion_package` WHERE `server_id` = %d AND `present` = 0;"
        % serverId,
    )
    db.commit()
    ...
    __doQuery(
        cursor,
        "UPDATE `minion_package` SET `present` = 0 WHERE `server_id` = %d" % serverId,
    )
    db.commit()
    ...
    for package, versions in __items(properties["pkgs"]):
        ...
        if not pkgId:
            __doQuery(
                cursor,
                'INSERT INTO `package` (`package_name`) VALUES ("%s");' % package,
            )
        ...
        for v in versions:
            ...
            if isinstance(v, dict) and "version" in v:
                version = v["version"]
            elif (not IS_PYTHON_3 and isinstance(v, basestring)) or (
                IS_PYTHON_3 and isinstance(v, str)
            ):
                version = v
            else:
                log.error(
                    "inventory.audit: could not process %s version %s" % (package, v)
                )
                continue
            __doQuery(
                cursor,
    ...
    __doQuery(
        cursor,
        "UPDATE `minion` SET `package_total` = (SELECT COUNT(*) FROM `minion_package` WHERE `server_id` = %d) WHERE `server_id` = %d;"
        % (serverId, serverId),
    )
    db.commit()
    return True
    ...
    for m in minions:
        try:
            ...
            if serverId:
                ...
                __doQuery(
                    cursor,
                    'UPDATE `minion` SET `last_seen` = "%s" WHERE `server_id` = %d'
                    % (ts, serverId),
                )
                db.commit()
            else:
                log.info(
                    "inventory.present: minion %s has not been audited, invoking audit"
                    % m
                )
                ...
                if "salt.execute" in __salt__:
                    ...
                    rtn = subprocess.call(
                        "salt '%s' inventory.audit force=True" % m, shell=True
                    )
                    if rtn == 0:
                        return True
